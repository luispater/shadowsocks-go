...
func (pm *PasswdManager) updatePortPasswd(host, port, password string, auth bool) {
	pl, ok := pm.get(port)
	if !ok {
	...
	go run(host, port, password, auth)
	if udp {
...
func updatePasswd(host string) {
	...
	for port, passwd := range config.PortPassword {
		passwdManager.updatePortPasswd(host, port, passwd, config.Auth)
...
func run(host, port, password string, auth bool) {
...
func main() {
	...
	flag.StringVar(&pidFile, "pid", "/var/run/shadowsocks-server.pid", "daemon mode pid file")
	flag.StringVar(&logFile, "log", "/var/log/shadowsocks-server.log", "daemon mode log file")
	...
	if err != nil {
		...
		cmdConfig.Server = config.Server
	...
	for port, password := range config.PortPassword {
		...
		go run(config.Server.(string), port, password, config.Auth)
		if udp {
	...
	if managerAddr != "" {
		...
		defer conn.Close()
		go managerDaemon(config.Server.(string), conn)
	}
	...
	waitSignal(config.Server.(string))
}

func managerDaemon(host string, conn *net.UDPConn) {
	...
	for {
		...
		case strings.HasPrefix(command, "add:"):
			res = handleAddPort(host, bytes.Trim(data[4:], "\x00\r\n "))
...
func handleAddPort(host string, payload []byte) []byte {
	var params struct {
	...
	passwdManager.updatePortPasswd(host, port, params.Password, config.Auth)
	return []byte("ok")
}

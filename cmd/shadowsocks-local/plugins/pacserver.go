...
func RunPacServer(config *ss.Config) {
	...
	if err != nil {
		...
		PacScript = fmt.Sprintf("var proxy = \"SOCKS5 %s:%d; SOCKS %s:%d; DIRECT;\";\n", config.LocalAddress, config.LocalPort, config.LocalAddress, config.LocalPort)
		PacScript = PacScript + "var rules = [\n"
		arrayGfwList := strings.Split(GfwList, "\n")
		blnWrited := false
		for i := range arrayGfwList {
			if i > 0 {
				strRule := strings.Trim(arrayGfwList[i], " \r")
				if len(strRule) != 0 && strRule[0:1] != "!" {
					if blnWrited {
						PacScript = PacScript + fmt.Sprintf(",\n")
					}
					strRule = strings.Replace(strRule, `\`, `\\`, -1)
					strRule = strings.Replace(strRule, `/`, `\/`, -1)
					PacScript = PacScript + fmt.Sprintf("\"%s\"", strRule)
					blnWrited = true
				}
			}
		}
		PacScript = PacScript + "\n];\n"
		PacScript = PacScript + PacScriptFooter

		http.HandleFunc("/", PacServerHandler)
		httpProxyListenAddr := fmt.Sprintf("%s:%d", config.LocalAddress, config.PacPort)
		log.Printf("starting local auto proxy configure http server at %v ...\n", httpProxyListenAddr)
		log.Fatal(http.ListenAndServe(httpProxyListenAddr, nil))
	}
}

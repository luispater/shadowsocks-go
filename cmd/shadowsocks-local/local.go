...
import (
	...
	"flag"
	"fmt"
	"github.com/luispater/go-ntlm/ntlm"
	"github.com/luispater/goproxy"
	"github.com/luispater/shadowsocks-go/cmd/shadowsocks-local/plugins"
...
var NtlmServerSessionV1 ntlm.ServerSession
var NtlmServerSessionV2 ntlm.ServerSession
...
func handShake(conn net.Conn) (err error) {
	...
	if (strings.Index(string(buf[2:nmethod+2]), string(0x0)) != -1) || strings.Index(string(buf[2:nmethod+2]), string(0x1)) != -1 || strings.Index(string(buf[2:nmethod+2]), string(0x2)) != -1 {
		if len(servers.socketPassword) == 0 {
			...
			if _, err := io.ReadAtLeast(conn, user, userLen); err != nil {
				conn.Write([]byte{0x1, 0x1})
				return err
			}

			// Get the password length
			if _, err := conn.Read(header[:1]); err != nil {
				_, err = conn.Write([]byte{0x1, 0x1})
				return err
			}
			...
			if password, hasKey := servers.socketPassword[string(user)]; hasKey {
				if strings.Index(string(buf[2:nmethod+2]), string(0x1)) != -1 || strings.Index(string(buf[2:nmethod+2]), string(0x2)) != -1 {
					if password == string(pass) {
						conn.Write([]byte{0x1, 0x0})
						return nil
					}
				} else {
					conn.Write([]byte{0x1, 0x0})
...
var servers struct {
	...
	socketPassword map[string]string
}
...
func parseServerConfig(config *ss.Config) {
	...
	servers.socketPassword = config.SocketPassword
...
func connectToServer(serverId int, rawaddr []byte, addr string) (remote *ss.Conn, err error) {
	...
	var obfs *ss.Obfs
	remote, err = ss.DialWithRawAddr(rawaddr, se.server, se.cipher.Copy(), obfs)
	if err != nil {
...
func main() {
	...
	flag.StringVar(&pidFile, "pid", "/var/run/shadowsocks-local.pid", "daemon mode pid file")
	flag.StringVar(&logFile, "log", "/var/log/shadowsocks-local.log", "daemon mode log file")
	...
	if daemonMode || config.Daemon {
		if config.DaemonPid != "" {
			pidFile = config.DaemonPid
		}
		if config.DaemonLog != "" {
			logFile = config.DaemonLog
		}
	...
	if config.HttpProxy {
		NtlmServerSessionV1, err = ntlm.CreateServerSession(ntlm.Version1, ntlm.ConnectionOrientedMode)
		NtlmServerSessionV2, err = ntlm.CreateServerSession(ntlm.Version2, ntlm.ConnectionOrientedMode)
		for userName := range servers.socketPassword {
			NtlmServerSessionV1.SetUserInfo(userName, servers.socketPassword[userName], "")
			NtlmServerSessionV2.SetUserInfo(userName, servers.socketPassword[userName], "")
		}
		...
		proxy.Tr = &http.Transport{
			DialContext: shadowSocksHttpProxyDialContext(config),
		}
		proxy.ConnectDial = shadowSocksHttpProxyDial(config)

		proxy.OnRequest().Do(plugins.HttpProxyAuth("auth", func(authType string, authInfo ...string) bool {
			if len(authInfo) < 2 {
				return false
			}
			if password, hasKey := servers.socketPassword[authInfo[0]]; hasKey {
		...
		proxy.OnRequest().HandleConnect(plugins.HttpProxyConnectAuth("auth", func(authType string, authInfo ...string) bool {
			if len(authInfo) < 2 {
				return false
			}
			if password, hasKey := servers.socketPassword[authInfo[0]]; hasKey {
		...
		httpProxyListenAddr := fmt.Sprintf("%s:%d", config.LocalAddress, config.HttpPort)
		go http.ListenAndServe(httpProxyListenAddr, &proxy)
		log.Printf("starting local http proxy server at %v ...\n", httpProxyListenAddr)
	}
	if config.Forword {
		go plugins.RunForward(config)
	}
	if config.Pac {
		go plugins.RunPacServer(config)
	}
	run(config.LocalAddress + ":" + strconv.Itoa(config.LocalPort))
}

func shadowSocksHttpProxyDial(config *ss.Config) func(_, addr string) (net.Conn, error) {
	return func(_, addr string) (net.Conn, error) {
		...
		serverAddr := net.JoinHostPort(config.Server.(string), strconv.Itoa(config.ServerPort))
...
func shadowSocksHttpProxyDialContext(config *ss.Config) func(_ context.Context, network, addr string) (net.Conn, error) {
	return func(_ context.Context, network, addr string) (net.Conn, error) {
		return shadowSocksHttpProxyDial(config)(network, addr)
	}
}
